...
# Make sure that all the necessary directories are included in line 42
...
# preference list
clang_format_tool_candiates=(clang-format)
...
# run clang-format
format_call="find src/ tests/ include/ -regextype egrep -regex '.+\.(h|hpp|cpp|cu|cuh)$' | xargs $clang_format_tool -i -style=file"
eval "$format_call"
